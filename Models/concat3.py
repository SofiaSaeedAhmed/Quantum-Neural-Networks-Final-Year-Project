...
class QuantumCNN(nn.Module):
    def __init__(self):
        ...
        # Fully connected layers - now outputting 50 features (10 quantum layers * 5 qubits)
        self.fc1 = nn.Linear(256 * 2 * 2, 512)  # Increased capacity
        self.bn_fc1 = nn.BatchNorm1d(512)
        self.fc2 = nn.Linear(512, 256)  # Additional layer for better feature transformation
        self.bn_fc2 = nn.BatchNorm1d(256)
        self.fc3 = nn.Linear(256, 50)  # 10 layers * 5 qubits = 50
        self.bn_fc3 = nn.BatchNorm1d(50)

        self.fc_output = nn.Linear(50, 10)  # Final output layer

        # Quantum layers - now 10 instances
        self.quantum_layers = nn.ModuleList([
            qml.qnn.TorchLayer(quantum_circuit, param_shapes)
            for _ in range(10)
        ])
    ...
    def forward(self, x):
        ...
        # Split into 10 parts (5 features each)
        x_split = torch.split(x, 5, dim=1)

        # Process each split through 10 quantum layers
        quantum_outputs = []
        for i in range(10):
            quantum_outputs.append(self.quantum_layers[i](x_split[i]))
...
log_file = os.path.join(output_dir, "concat3_output.txt")
...
model_path = os.path.join(output_dir, "concat3_model.pth") # ------------------------------ CHANGE !!!!
