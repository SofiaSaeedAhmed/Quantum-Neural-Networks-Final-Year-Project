...
# Load Medical MNIST dataset0.0026
...
if __name__ == "__main__":
    ...
    quantum_params = [p for name, p in model.named_parameters() if "quantum_layer" in name]
    classical_params = [p for name, p in model.named_parameters() if not "quantum_layer" in name]

    quantum_optimizer = optim.Adam(quantum_params, lr=0.005)
    ...
    log_file = os.path.join(output_dir, "medicalquantum_output.txt")
    ...
    model_path = os.path.join(output_dir, "medicalquantum_model.pth")       # --------------------------------------------------------
