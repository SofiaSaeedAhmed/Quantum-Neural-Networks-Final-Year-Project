...
def quantum_circuit(inputs, weights, entangling_weights):
    ...
    # Strongly entangling layer using entangling weights
    qml.StronglyEntanglingLayers(entangling_weights, wires=range(num_qubits))

    # Measurement: expectation values of Pauli-Y for each qubit
    return [qml.expval(qml.PauliX(i)) for i in range(num_qubits)]

# Define shapes for weights (note: StronglyEntanglingLayers expects different weight shape)
param_shapes = {"weights": (num_layers, num_qubits, 3),"entangling_weights": (L, num_qubits, 3)}
...
log_file = os.path.join(output_dir, "cifar6_output.txt")
...
model_path = os.path.join(output_dir, "cifar6_model.pth")
