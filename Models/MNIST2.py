...
# Quantum node for a single PQC2, replace with other PQC architecture
...
log_file = os.path.join(output_dir, "mnist2_output.txt")
...
model_path = os.path.join(output_dir, "mnist2_model.pth")
...
def visualize_and_save_predictions(model, val_loader, num_images=20, save_path="mnistPredictions.png"):
    # Get random batch
    shuffled_loader = DataLoader(val_loader.dataset, batch_size=num_images, shuffle=True)
    images, labels = next(iter(shuffled_loader))
    ...
    # Create figure
    fig, axes = plt.subplots(4, 5, figsize=(15, 12))  # 4 rows x 5 columns
    plt.suptitle("Model Predictions on MNIST Test Set", fontsize=16, y=1.02)

    for idx, ax in enumerate(axes.flat):
        if idx >= num_images:
            break

        img = images[idx].numpy().squeeze()  # Remove channel dim for MNIST
...
visualize_and_save_predictions(
    ...
    num_images=20,  # Will show 20 images (4x5 grid)
    save_path=os.path.join(output_dir, "mnist_predictions.png")  # Save in your output directory
)